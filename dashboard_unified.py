...
0.1em; margin: 24px 0 10px 0; padding-bottom: 6px; border-bottom: 1px solid #e5e7eb; }
  .mode-description { background: white; border-radius: 8px; padding: 12px 16px; border-left: 3px solid #4f7cff; 
margin-bottom: 1rem; font-size: 0.82rem; color: #374151; line-height: 1.6; }
...
def recalculate_rul(site_result, new_failure_dt):
  """Recalculate RUL for a site using a custom failure DeltaT threshold."""
  ...
  if r2 < 0.25 or slope <= 0:
      ...
      return site_copy
  if slope > 0:
      episodes_to_failure = (new_failure_dt - current_dt) / slope
  else:
      episodes_to_failure = 999
...
'WARNING', 'OK'], label_visibility='visible')
  ...
  st.markdown("---")
  st.markdown('<div style="font-size:0.72rem;color:#9ca3af;line-height:1.6;">💡 Analysis assumes 90 days of InfluxDB data.<br>RUL projects when rolling median hits failure threshold.<br>Green = filter healthy. Yellow = plan replacement soon. Red = replace now.</div>', unsafe_allow_html=True)
...
**📈 Trend Analysis**
- R²: {r2_val:.3f}
- Slope: {slope_val:.3f} °C/episode

**⏱️  RUL Estimate**
- Days: {rul_str}
- % Life Used: {pct_life_val:.0f}%

**🌡️  Temperature**
- Current ΔT: {current_dt_val:.1f}°C
- Failure ΔT: {failure_dt_val:.1f}°C
- Baseline ΔT: {baseline_dt_val:.1f}°C
"""
              st.markdown(site_info_text)
          with col2:
...
footer_text = f"Last updated: {query_ts}<br>Query time: {query_elapsed:.1f}s for {sites_queried} sites<br>Dashboard: Mode 3 Rolling Median RUL Analysis"
st.markdown(f'<div style="text-align:center;font-size:0.75rem;color:#9ca3af;">{footer_text}</div>', unsafe_allow_html=True)
