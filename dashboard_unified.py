...
Displays Mode 3 (adjusted runtime hours) analysis.
Reads from sites_data (1).json (generated by query_sites.py).
"""
...
def recalculate_rul(site_result, new_failure_dt):
  ...
  Uses adjusted runtime hours (fan_speed² weighted) instead of episode count.
...
with st.sidebar:
  ...
  st.markdown('<div style="font-size:0.72rem;color:#9ca3af;line-height:1.6;">'
              '💡 Analysis uses max ΔT per episode vs adjusted runtime hours (fan²-weighted).<br>'
              'RUL projects when trend hits failure threshold.<br>'
...
if not detail_sites:
  ...
  for site_id, result in detail_sites:
      ...
      with st.expander(f"{emoji} {site_id} — {result.get('site_name', '?')} (RUL: {rul:.0f}d)" if isinstance(rul,
float) else f"{emoji} {site_id} — {result.get('site_name', '?')} (RUL: {rul})"):
...
**📈 Trend Analysis**
- R²: {result.get('r2', '?'):.3f}
- Slope: {result.get('slope', '?'):.4f} °C/adjusted-hour
...
""")
          ...
          with col2:
              # Plot max ΔT vs cumulative adjusted hours
              ...
              if len(max_deltas) > 2 and len(cumulative_adjusted_hours) > 2:
                  ...
                  # Rolling median
                  fig.add_trace(go.Scatter(
                      x=cumulative_adjusted_hours, y=rolling_median,
                      mode='lines+markers',
                      line=dict(color='#f59e0b', width=2.5),
                      marker=dict(size=5, color='#f59e0b'),
                      name='Rolling Median',
                      hovertemplate='Adjusted Hours: %{x:.1f}<br>Rolling Median: %{y:.2f}°C<extra></extra>'
                  ))
                  ...
                  if len(cumulative_adjusted_hours) >= 2:
                      ...
                      coeffs = np.polyfit(cumulative_adjusted_hours, rolling_median, 1)
...
<div style="text-align:center;font-size:0.75rem;color:#9ca3af;">
  ...
  Dashboard: Mode 3 Max ΔT vs Adjusted Runtime Hours Analysis
</div>
""", unsafe_allow_html=True)
