...
<style>
    ...
    [data-testid="stDataFrame"] th { background-color: #f3f4f6 !important; color: #1a202c !important; border: 1px solid #d1d5db !important; padding: 12px !important; }
    [data-testid="stDataFrame"] td { background-color: #ffffff !important; color: #1a202c !important; border: 1px solid #d1d5db !important; padding: 12px !important; }
        ...
        with st.expander(expander_title):
            ...
            with col1:
                ...
                if isinstance(r2_check, (int, float)) and r2_check < 0.25:
                    st.warning("⚠️  Data quality is insufficient to make an accurate prediction (R² < 0.25)")
                elif isinstance(slope_check, (int, float)) and slope_check <= 0:
                    st.warning("⚠️  Filter is not degrading or is improving - prediction is not accurate")
                else:
...
footer_html = f'<div style="text-align:center;font-size:0.75rem;color:#9ca3af;margin-top:32px;">Last updated: {query_ts}<br>Query time: {query_elapsed:.1f}s for {sites_queried} sites<br>Mode 3 Rolling Median RUL Analysis</div>'
