...
Reads from sites_data (2).json (generated by query_sites.py).
"""
...
def load_sites_data(json_file='sites_data (2).json'):
    if not Path(json_file).exists():
        st.error(f"Data file not found: {json_file}")
        st.info("Run python query_sites.py on the Bell laptop to generate this file.")
        st.stop()
...
def recalculate_rul(site_result, new_failure_dt):
    ...
    if r2 < 0.25 or slope <= 0:
        ...
        if r2 < 0.25 and slope <= 0:
            site_copy['data_reason'] = 'R2 less than 0.25 (insufficient data quality) and negative or flat trend'
        elif r2 < 0.25:
            site_copy['data_reason'] = 'R2 less than 0.25 (insufficient data quality)'
        else:
            site_copy['data_reason'] = 'Filter is not degrading or is improving (negative or flat trend)'
...
with st.sidebar:
    st.markdown("### Rogers HVAC RUL Dashboard")
    st.markdown("---")
    ...
    search_term = st.text_input("Search site name or ID", "", label_visibility='visible')
...
st.markdown(f"## Rogers HVAC Filter RUL Status - {len([s for s in sites_recalc.values() if s.get('success')])} Sites Analyzed")
...
with col1:
    ...
    st.markdown(f'<div class="metric-card danger"><div class="label">URGENT</div><div class="value">{urgent_count}</div><div class="sub">less than 14 days</div></div>', unsafe_allow_html=True)

with col2:
    warning_count = sum(1 for s in sites_recalc.values() if s.get('success') and s.get('urgency') == 'WARNING')
    st.markdown(f'<div class="metric-card warn"><div class="label">WARNING</div><div class="value">{warning_count}</div><div class="sub">14 to 30 days</div></div>', unsafe_allow_html=True)

with col3:
    ok_count = sum(1 for s in sites_recalc.values() if s.get('success') and s.get('urgency') == 'OK')
    st.markdown(f'<div class="metric-card"><div class="label">OK</div><div class="value">{ok_count}</div><div class="sub">30 or more days</div></div>', unsafe_allow_html=True)

with col4:
    no_data_count = sum(1 for s in sites_recalc.values() if s.get('success') and s.get('urgency') == 'NO_DATA')
    st.markdown(f'<div class="metric-card"><div class="label">NO DATA</div><div class="value">{no_data_count}</div><div class="sub">Cannot extrapolate</div></div>', unsafe_allow_html=True)
...
for site_id, result in sites_recalc.items():
    ...
    table_data.append({
        ...
        'R2': f"{result.get('r2', 0):.3f}",
        'Current Delta T (C)': f"{result.get('current_dt', 0):.1f}",
        'Failure Delta T (C)': f"{result.get('failure_dt', 0):.1f}",
...
st.markdown(f'<div class="section-header">Site Details and Trend Analysis</div>', unsafe_allow_html=True)
...
if not detail_sites:
    ...
    for site_id, result in detail_sites:
        ...
        color_map = {'URGENT': 'RED', 'WARNING': 'YELLOW', 'OK': 'GREEN', 'UNKNOWN': 'GRAY'}
        emoji = color_map.get(urgency, 'QUESTION')

        rul_display = f"{rul:.0f}d" if isinstance(rul, float) else "N/A"
        with st.expander(f"{site_id} - {result.get('site_name', '?')} (RUL: {rul_display})"):
...
**Trend Analysis**
R2: {result.get('r2', '?'):.3f}
Slope: {result.get('slope', '?'):.4f} C per adjusted hour

**Unable to Extrapolate**
{result.get('data_reason', 'Insufficient data')}

**Temperature**
Current Delta T: {result.get('current_dt', '?'):.1f} C
Failure Delta T: {result.get('failure_dt', '?'):.1f} C
Baseline Delta T: {result.get('baseline_dt', '?'):.1f} C""")
                else:
                    st.markdown(f"""**Site Info**
IP: {result.get('ip')}
Episodes: {result.get('episodes_count')}
...
Percent Life Used: {result.get('pct_life', '?'):.0f}%
Avg Runtime: {result.get('avg_adjusted_hours_per_day', '?'):.2f} hours per day
                ...
                if urgency != 'NO_DATA':
                    ...
                    if len(max_deltas) > 2 and len(cumulative_adjusted_hours) > 2:
                        ...
                        fig.add_trace(go.Scatter(
                            x=cumulative_adjusted_hours,
                            y=max_deltas,
                            mode='markers',
                            marker=dict(size=7, color='lightblue', opacity=0.7, line=dict(color='steelblue', width=1)),
                            name='Max Delta T raw',
                            hovertemplate='Adjusted Hours: %{x:.1f}<br>Max Delta T: %{y:.2f}C<extra></extra>'
                        ))
                        ...
                        if len(cumulative_adjusted_hours) >= 2:
                            ...
                            fig.add_trace(go.Scatter(
                                ...
                                name=f'Linear Trend R2={r2:.3f}',
                                hovertemplate='Adjusted Hours: %{x:.1f}<br>Trend: %{y:.2f}C<extra></extra>'
                            ))

                        fig.update_layout(
                            title=dict(
                                text=f"Max Delta T Degradation Trend - {site_id}",
                                font=dict(color='#1a202c', size=14)
                            ),
                            xaxis_title='Cumulative Adjusted Runtime Hours (fan squared weighted)',
                            xaxis=dict(
...
st.markdown("---")
st.markdown(f"""<div style="text-align:center;font-size:0.75rem;color:#9ca3af;">Last updated: {data.get('query_timestamp', 'unknown')}<br>Query time: {data.get('query_elapsed_seconds', '?'):.1f} seconds for {data.get('sites_queried', '?')}
sites<br>Dashboard: Mode 3 Max Delta T vs Adjusted Runtime Hours (fan squared weighted) - Linear regression on raw data with Weatherbit 90-day air quality</div>""", unsafe_allow_html=True)
